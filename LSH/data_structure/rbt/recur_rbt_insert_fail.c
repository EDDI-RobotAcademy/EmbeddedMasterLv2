...
#if 0
	1) parent가 RED 인지 검사(매 재귀마다 반복해야함) - while로 처리할 필요가 있는가 ?
	2) parent가 grand_parent의 좌측인지 검사

	3) 삼촌의 색상이 Red인지 검사
	4) 삼촌이 Red라면 삼촌은 Black, 조부모는 Red

	5) root가 parent 우측에 있는지 검사
	6) 우측에 있다면 Left Rotation

	7) 1번 로직의 검사 이후(false라면 재귀 종료)
	8) 만약 다시 들어가게 된다면 
#endif
	if (real_root_flag)
	{
...
int main(void)
{
	...
	int data[16384] = {
		...
		15, 10, 27, 14, 19, 1, 20, 28, 8, 23,
		5, 16, 26, 2, 11, 7, 3, 21, 30, 9
	};
	int del_data[16384] = {
